...
export default function AdminShopsPage() {
  ...
  const { data: shopsResponse, isLoading, error, refetch } = useAllShops();
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'pending':
        return <Badge variant="info">Đang chờ</Badge>;
      default:
    ...
    <div className="space-y-6">
      ...
      <AdminStatsGrid className="lg:grid-cols-3">
        <AdminStatCard
          ...
          value={totalShops}
          icon={Store}
          description="Tất cả cửa hàng đã đăng ký"
        />
        <AdminStatCard
          ...
          icon={CheckCircle}
          accent="green"
          description="Shop có thể kinh doanh bình thường"
        />
        <AdminStatCard
          title="Tạm dừng"
          value={inactiveShops}
          icon={Ban}
          accent="amber"
          description="Shop đang bị tắt hoặc chờ xử lý"
        />
      </AdminStatsGrid>
      ...
      {/* View Shop Modal */}
      <ViewShopModal isOpen={isModalOpen} onClose={handleCloseModal} shop={selectedShop} />
    </div>
  );
}
