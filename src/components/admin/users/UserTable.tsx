          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className={adminMenuContentClass}>
              ...
              <DropdownMenuItem
                ...
                className={`cursor-pointer rounded-lg ${!selectedRole ? 'bg-muted font-medium' : ''}`}
              >
                Tất cả vai trò
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => handleRoleFilter('admin')}
                className={`cursor-pointer rounded-lg ${selectedRole === 'admin' ? 'bg-muted font-medium' : ''}`}
              >
                Quản trị viên
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => handleRoleFilter('user')}
                className={`cursor-pointer rounded-lg ${selectedRole === 'user' ? 'bg-muted font-medium' : ''}`}
              >
                Người dùng
              </DropdownMenuItem>
            </DropdownMenuContent>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className={adminMenuContentClass}>
              ...
              <DropdownMenuItem
                ...
                className={`cursor-pointer rounded-lg ${selectedVerified === null ? 'bg-muted font-medium' : ''}`}
              >
                Tất cả trạng thái
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => handleVerifiedFilter(true)}
                className={`cursor-pointer rounded-lg ${selectedVerified === true ? 'bg-muted font-medium' : ''}`}
              >
                Đã xác minh
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => handleVerifiedFilter(false)}
                className={`cursor-pointer rounded-lg ${selectedVerified === false ? 'bg-muted font-medium' : ''}`}
              >
                Chưa xác minh
              </DropdownMenuItem>
            </DropdownMenuContent>
                  ...
                  <TableRow key={user._id} className={`${adminRowHoverClass} border-0`}>
                    <TableCell className="pl-6 font-medium">
                      <div className="flex items-center gap-3">
                        <div
                          className={`relative w-10 h-10 rounded-full overflow-hidden ${adminMediaPlaceholderClass}`}
                        >
                          {user.avatar ? (
                            <Image
                    ...
                    <TableCell>{getVerifiedBadge(user.isVerifiedEmail)}</TableCell>
                    <TableCell>
