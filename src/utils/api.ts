...
const BACKEND_MESSAGE_TRANSLATIONS: Record<string, string> = {
  ...
  'two-factor authentication is already enabled': 'Xác thực 2 yếu tố đã được bật',
  'two-factor authentication is not enabled': 'Xác thực 2 yếu tố chưa được bật',
  'failed to send two-factor authentication code. please try again.':
    'Không thể gửi mã xác thực 2 yếu tố. Vui lòng thử lại',
  'invalid or expired two-factor challenge': 'Phiên xác thực 2 yếu tố không còn hiệu lực',
  ...
  'invalid or revoked refresh token': 'Phiên đăng nhập không còn hiệu lực, vui lòng đăng nhập lại',
...
export function getSafeErrorMessage(
  ...
  if (axiosLikeError?.code === 'ECONNABORTED' || axiosLikeError?.response?.status === 408) {
...
function hasNestedData<T>(data: unknown): data is ApiResponse<T> {
  return data !== null && typeof data === 'object' && 'data' in data && 'status' in data;
}
...
export function extractPaginatedData<T>(response: {
  data: ApiResponse<PaginatedResponse<T>> | PaginatedResponse<T>;
}): PaginatedResponse<T> {
  const data = response.data;
