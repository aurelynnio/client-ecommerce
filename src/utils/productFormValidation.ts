...
export function validateProductForm(formData: ProductFormValidationInput): string | null {
  ...
  if (!Number.isFinite(formData.price.currentPrice) || formData.price.currentPrice < 0) {
    return 'Giá bán không hợp lệ.';
  }

  if (formData.price.discountPrice !== undefined && formData.price.discountPrice !== null) {
    if (!Number.isFinite(formData.price.discountPrice) || formData.price.discountPrice < 0) {
